...
import { ABILITY_LABELS, SKILL_TO_ABILITY } from "../lib/constants.js";
...
export function MockPlayerView({ broadcast, onRequestRedraw, mockPlayerId }) {
  ...
  useEffect(() => {
    if (!broadcast) return;
    if (broadcast.type === "diceRolled") {
      startDiceRoll(broadcast.data);
    } else if (broadcast.type === "cardDrawn") {
      applyDraw(broadcast.data);
    } else if (broadcast.type === "cardResolved") {
      clearDraw();
    }
  }, [broadcast, applyDraw, startDiceRoll, clearDraw]);

  const requestRedraw = () => {
    if (!lastDraw) return;
    setRedrawing(true);
    redrawTimer.current = setTimeout(() => setRedrawing(false), 3000);
    if (onRequestRedraw) onRequestRedraw();
  };
  ...
  const isMyDraw = !mockPlayerId || lastDraw?.playerId === mockPlayerId;
      ...
      <div className="player-view-center">
        ...
        {lastDraw && (
          <>
            ...
            {proficiency > 0 && !skipReason && isMyDraw && (
